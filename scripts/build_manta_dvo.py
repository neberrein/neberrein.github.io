...
def results_html(summary, figures, mobile_figures, asset_path):
    ...
    block += [f'<p>SimpleTracking 조건에서는 12개 중 {totals[MODES[0]]}개, PNG 조건에서는 12개 중 {totals[MODES[1]]}개에서 최종 회피를 확인했습니다. 이 시험에서는 접근 방향뿐 아니라 위협의 유도 방식에 따라 결과 차이가 나타났습니다.</p>',
